...
def train(
    ...
    for t in range(epochs):
        train_acc, train_loss = train_model(
            model, train_data, loss_fn, optimizer, batch_size, dtype
        )
...
def run_trial(trial: optuna.Trial):
    ...
    print(config_vals)
    if WANDB_ACTIVE:
        wandb.init(project="mnist-example-snntorch", config=config_vals)
    train_data, test_data = load_data(batch_size=config_vals["batch_size"])
    model = SimpleNetwork(
        28 * 28, config_vals["num_hidden"], 10, config_vals["beta"], 25
    ).to(device)
